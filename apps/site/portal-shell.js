...
(function () {
    ...
    document.querySelectorAll("[data-tabs]").forEach((group) => {
      ...
      btns.forEach((b) => {
        b.addEventListener("click", () => {
          ...
          document
            .querySelectorAll(`[data-tab-panel][data-tabs-group="${name}"]`)
            .forEach((p) => p.classList.remove("active"));
          b.classList.add("active");
          const target = document.querySelector(
            `[data-tab-panel="${b.dataset.tab}"][data-tabs-group="${name}"]`,
          );
  ...
  function bindModals() {
    ...
    document.addEventListener("keydown", (e) => {
      if (e.key === "Escape") {
        document
          .querySelectorAll(".modal-backdrop.open, .sheet.open, .sheet-backdrop.open")
  ...
  window.openSheet = (id) => {
    const s = document.getElementById(id),
      bd = document.getElementById(id + "-backdrop");
  ...
  function bindSheets() {
    document
      ...
      .forEach((b) => b.addEventListener("click", () => openSheet(b.dataset.openSheet)));
    document.querySelectorAll("[data-close-sheet]").forEach((b) =>
      b.addEventListener("click", () => {
        const s = b.closest(".sheet");
        if (s) closeSheet(s.id);
      }),
    );
    document
      .querySelectorAll(".sheet-backdrop")
      .forEach((bd) =>
        bd.addEventListener("click", () => closeSheet(bd.id.replace("-backdrop", ""))),
      );
  }
  ...
  function bindCopy() {
    document.querySelectorAll(".copy[data-copy]").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        btn.innerHTML =
          '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.6"><path d="M3 8.5l3 3 7-7" stroke-linecap="round" stroke-linejoin="round"/></svg> copied';
        setTimeout(() => {
