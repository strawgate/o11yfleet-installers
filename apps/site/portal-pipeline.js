...
(function () {
  ...
  const CATALOG = {
    receivers: [
      { type: 'otlp', desc: 'OTLP gRPC + HTTP', signals: ['logs','metrics','traces'], fields: [
        { k: 'protocols.grpc.endpoint', v: '0.0.0.0:4317' },
        { k: 'protocols.http.endpoint', v: '0.0.0.0:4318' },
      ]},
      { type: 'hostmetrics', desc: 'Host CPU/mem/disk', signals: ['metrics'], fields: [
        { k: 'collection_interval', v: '30s' },
        { k: 'scrapers', v: 'cpu, memory, disk, network, load' },
      ]},
      { type: 'filelog', desc: 'Log files', signals: ['logs'], fields: [
        { k: 'include', v: '[/var/log/*.log]' },
        { k: 'start_at', v: 'end' },
      ]},
      { type: 'kubeletstats', desc: 'Kubelet metrics', signals: ['metrics'], fields: [
        { k: 'collection_interval', v: '20s' },
        { k: 'auth_type', v: 'serviceAccount' },
      ]},
      { type: 'prometheus', desc: 'Prom scrape', signals: ['metrics'], fields: [
        { k: 'config.scrape_configs[0].job_name', v: 'app' },
        { k: 'config.scrape_configs[0].static_configs[0].targets', v: '[localhost:9090]' },
      ]},
      { type: 'k8s_events', desc: 'K8s events as logs', signals: ['logs'], fields: [
        { k: 'namespaces', v: '[default, kube-system]' },
      ]},
      { type: 'jaeger', desc: 'Jaeger traces', signals: ['traces'], fields: [
        { k: 'protocols.grpc.endpoint', v: '0.0.0.0:14250' },
      ]},
    ],
    processors: [
      { type: 'memory_limiter', desc: 'Backpressure', signals: ['logs','metrics','traces'], fields: [
        { k: 'check_interval', v: '1s' },
        { k: 'limit_mib', v: '512' },
      ]},
      { type: 'batch', desc: 'Batch sends', signals: ['logs','metrics','traces'], fields: [
        { k: 'send_batch_size', v: '8192' },
        { k: 'timeout', v: '10s' },
      ]},
      { type: 'k8sattributes', desc: 'Enrich w/ K8s', signals: ['logs','metrics','traces'], fields: [
        { k: 'extract.metadata', v: '[k8s.namespace.name, k8s.pod.name, k8s.deployment.name]' },
        { k: 'passthrough', v: 'false' },
      ]},
      { type: 'resourcedetection', desc: 'Add resource attrs', signals: ['logs','metrics','traces'], fields: [
        { k: 'detectors', v: '[env, system, ec2]' },
      ]},
      { type: 'attributes', desc: 'Set / drop attrs', signals: ['logs','metrics','traces'], fields: [
        { k: 'actions[0].key', v: 'env' },
        { k: 'actions[0].value', v: 'production' },
        { k: 'actions[0].action', v: 'insert' },
      ]},
      { type: 'filter', desc: 'Drop by query', signals: ['logs','metrics','traces'], fields: [
        { k: 'logs.log_record', v: '[severity_number < SEVERITY_NUMBER_INFO]' },
      ]},
      { type: 'transform', desc: 'OTTL transforms', signals: ['logs','metrics','traces'], fields: [
        { k: 'log_statements[0].context', v: 'log' },
        { k: 'log_statements[0].statements', v: '[set(severity_text, "info") where severity_text == ""]' },
      ]},
      { type: 'tail_sampling', desc: 'Sample traces', signals: ['traces'], fields: [
    ...
    exporters: [
      { type: 'otlp', desc: 'OTLP gRPC out', signals: ['logs','metrics','traces'], fields: [
        { k: 'endpoint', v: 'otelcol-gateway:4317' },
        { k: 'tls.insecure', v: 'true' },
      ]},
      { type: 'otlphttp', desc: 'OTLP HTTP out', signals: ['logs','metrics','traces'], fields: [
        { k: 'endpoint', v: 'https://api.honeycomb.io' },
        { k: 'headers.x-honeycomb-team', v: '${HONEYCOMB_API_KEY}' },
      ]},
      { type: 'prometheusremotewrite', desc: 'Prom remote write', signals: ['metrics'], fields: [
        { k: 'endpoint', v: 'https://prometheus.example.com/api/v1/write' },
      ]},
      { type: 'loki', desc: 'Loki logs', signals: ['logs'], fields: [
        { k: 'endpoint', v: 'https://logs-prod.grafana.net/loki/api/v1/push' },
      ]},
      { type: 'datadog', desc: 'Datadog', signals: ['logs','metrics','traces'], fields: [
        { k: 'api.site', v: 'datadoghq.com' },
        { k: 'api.key', v: '${DD_API_KEY}' },
      ]},
      { type: 'splunk_hec', desc: 'Splunk HEC', signals: ['logs','metrics','traces'], fields: [
        { k: 'endpoint', v: 'https://hec.example.com:8088/services/collector' },
        { k: 'token', v: '${SPLUNK_HEC_TOKEN}' },
      ]},
      { type: 'elasticsearch', desc: 'Elasticsearch', signals: ['logs','traces'], fields: [
        { k: 'endpoints', v: '[https://es.example.com:9200]' },
      ]},
      { type: 'debug', desc: 'Stdout debug', signals: ['logs','metrics','traces'], fields: [
        { k: 'verbosity', v: 'detailed' },
      ]},
    ],
  };
  ...
  const EXAMPLES = {
    'edge-gateway': {
      ...
      receivers: [
        { id: 'r1', type: 'otlp', name: 'otlp', signals: ['logs','metrics','traces'], cfg: { 'protocols.grpc.endpoint': '0.0.0.0:4317', 'protocols.http.endpoint': '0.0.0.0:4318' } },
      ],
      processors: [
        { id: 'p1', type: 'memory_limiter', name: 'memory_limiter', signals: ['logs','metrics','traces'], cfg: { check_interval: '1s', limit_mib: '1024' } },
        { id: 'p2', type: 'attributes', name: 'attributes/env', signals: ['logs','metrics','traces'], cfg: { 'actions[0].key': 'env', 'actions[0].value': 'production', 'actions[0].action': 'insert' } },
        { id: 'p3', type: 'batch', name: 'batch', signals: ['logs','metrics','traces'], cfg: { send_batch_size: '8192', timeout: '10s' } },
      ],
      exporters: [
        { id: 'e1', type: 'otlp', name: 'otlp/gateway', signals: ['logs','metrics','traces'], cfg: { endpoint: 'otelcol-gateway:4317', 'tls.insecure': 'true' } },
      ],
      wires: [
        ['r1','p1','logs'], ['r1','p1','metrics'], ['r1','p1','traces'],
        ['p1','p2','logs'], ['p1','p2','metrics'], ['p1','p2','traces'],
        ['p2','p3','logs'], ['p2','p3','metrics'], ['p2','p3','traces'],
        ['p3','e1','logs'], ['p3','e1','metrics'], ['p3','e1','traces'],
      ],
    },
    'k8s-daemon': {
      ...
      receivers: [
        { id: 'r1', type: 'filelog', name: 'filelog/containers', signals: ['logs'], cfg: { include: '[/var/log/pods/*/*/*.log]', start_at: 'end' } },
        { id: 'r2', type: 'kubeletstats', name: 'kubeletstats', signals: ['metrics'], cfg: { collection_interval: '20s', auth_type: 'serviceAccount' } },
        { id: 'r3', type: 'k8s_events', name: 'k8s_events', signals: ['logs'], cfg: { namespaces: '[]' } },
      ],
      processors: [
        { id: 'p1', type: 'k8sattributes', name: 'k8sattributes', signals: ['logs','metrics'], cfg: { 'extract.metadata': '[k8s.namespace.name, k8s.pod.name, k8s.deployment.name]' } },
        { id: 'p2', type: 'resourcedetection', name: 'resourcedetection', signals: ['logs','metrics'], cfg: { detectors: '[env, system]' } },
        { id: 'p3', type: 'batch', name: 'batch', signals: ['logs','metrics'], cfg: { send_batch_size: '4096', timeout: '5s' } },
      ],
      exporters: [
        { id: 'e1', type: 'loki', name: 'loki', signals: ['logs'], cfg: { endpoint: 'https://logs-prod.grafana.net/loki/api/v1/push' } },
        { id: 'e2', type: 'prometheusremotewrite', name: 'prometheusrw', signals: ['metrics'], cfg: { endpoint: 'https://prom.example.com/api/v1/write' } },
      ],
      wires: [
        ['r1','p1','logs'], ['r2','p1','metrics'], ['r3','p1','logs'],
        ['p1','p2','logs'], ['p1','p2','metrics'],
        ['p2','p3','logs'], ['p2','p3','metrics'],
        ['p3','e1','logs'], ['p3','e2','metrics'],
      ],
    },
    'host-monitor': {
      ...
      receivers: [
        { id: 'r1', type: 'hostmetrics', name: 'hostmetrics', signals: ['metrics'], cfg: { collection_interval: '30s', scrapers: 'cpu, memory, disk, network, load, filesystem' } },
      ],
      processors: [
        { id: 'p1', type: 'resourcedetection', name: 'resourcedetection', signals: ['metrics'], cfg: { detectors: '[env, system, ec2]' } },
        { id: 'p2', type: 'batch', name: 'batch', signals: ['metrics'], cfg: { send_batch_size: '2048', timeout: '10s' } },
      ],
      exporters: [
        { id: 'e1', type: 'datadog', name: 'datadog', signals: ['metrics'], cfg: { 'api.site': 'datadoghq.com', 'api.key': '${DD_API_KEY}' } },
      ],
      wires: [ ['r1','p1','metrics'], ['p1','p2','metrics'], ['p2','e1','metrics'] ],
    },
    'ingest-gateway': {
      label: 'Ingest gateway (regional)',
      desc: 'High-throughput aggregator. Tail samples traces, splits to multiple backends.',
      receivers: [
        { id: 'r1', type: 'otlp', name: 'otlp', signals: ['logs','metrics','traces'], cfg: { 'protocols.grpc.endpoint': '0.0.0.0:4317' } },
        { id: 'r2', type: 'jaeger', name: 'jaeger', signals: ['traces'], cfg: { 'protocols.grpc.endpoint': '0.0.0.0:14250' } },
      ],
      processors: [
        { id: 'p1', type: 'memory_limiter', name: 'memory_limiter', signals: ['logs','metrics','traces'], cfg: { check_interval: '1s', limit_mib: '4096' } },
        { id: 'p2', type: 'tail_sampling', name: 'tail_sampling', signals: ['traces'], cfg: { decision_wait: '10s', 'policies[0].name': 'errors', 'policies[0].type': 'status_code' } },
        { id: 'p3', type: 'transform', name: 'transform/redact', signals: ['logs'], cfg: { 'log_statements[0].context': 'log', 'log_statements[0].statements': '[replace_pattern(body, "(?i)password=[^ ]+", "password=***")]' } },
        { id: 'p4', type: 'batch', name: 'batch', signals: ['logs','metrics','traces'], cfg: { send_batch_size: '16384', timeout: '5s' } },
      ],
      exporters: [
        { id: 'e1', type: 'splunk_hec', name: 'splunk_hec', signals: ['logs'], cfg: { endpoint: 'https://hec.example.com:8088/services/collector' } },
        { id: 'e2', type: 'prometheusremotewrite', name: 'prometheusrw', signals: ['metrics'], cfg: { endpoint: 'https://prom.example.com/api/v1/write' } },
        { id: 'e3', type: 'datadog', name: 'datadog/traces', signals: ['traces'], cfg: { 'api.site': 'datadoghq.com', 'api.key': '${DD_API_KEY}' } },
      ],
      wires: [
        ['r1','p1','logs'], ['r1','p1','metrics'], ['r1','p1','traces'],
        ['r2','p1','traces'],
        ['p1','p3','logs'], ['p1','p4','metrics'], ['p1','p2','traces'],
        ['p3','p4','logs'], ['p2','p4','traces'],
        ['p4','e1','logs'], ['p4','e2','metrics'], ['p4','e3','traces'],
      ],
    },
  };
  ...
  const PIPELINE_INSIGHTS = {
    'edge-gateway': [
      { id:'i1', node:'p3', kind:'info', title:'Batch size could go higher', desc:'p99 batch fill is 31%. Bumping send_batch_size from 8192 → 16384 should reduce egress by ~12%.' },
    ],
    'k8s-daemon': [
      { id:'i1', node:'r3', kind:'warn', title:'k8s_events on all namespaces', desc:'Currently scoping to all namespaces (37). Limit to 4 active ones to drop log volume by ~80%.' },
      { id:'i2', node:'e2', kind:'err', title:'Remote write rejecting samples', desc:'2.3% of samples rejected for out-of-order timestamps in the last hour.' },
    ],
    'host-monitor': [],
    'ingest-gateway': [
      { id:'i1', node:'p2', kind:'info', title:'Tail-sampling head room', desc:'Current decision_wait=10s catches 94% of error spans. Dropping to 6s would barely change recall and free 1.2GB heap.' },
      { id:'i2', node:'p1', kind:'warn', title:'memory_limiter trips daily at 03:14', desc:'Spikes correlate with nightly batch jobs from cluster prod-east. Bumping limit_mib to 6144 stops drops.' },
    ],
  };
  ...
  const ICONS = {
    receiver: '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><path d="M3 8h7M7 5l3 3-3 3"/><circle cx="13" cy="8" r="1.5"/></svg>',
    processor: '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5"><rect x="3" y="3" width="10" height="10" rx="1.5"/><path d="M6 8h4M8 6v4" stroke-linecap="round"/></svg>',
    exporter: '<svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round"><circle cx="3" cy="8" r="1.5"/><path d="M5 8h7M9 5l3 3-3 3"/></svg>',
  };

  function sigPill(s) { return `<span class="sig sig-${s}">${s.slice(0,3)}</span>`; }

  function nodeHTML(n, role, hasInsight) {
    const sigs = (n.signals || []).map(sigPill).join('');
    const insightDot = hasInsight ? `<span class="insight ${hasInsight.kind}" title="${hasInsight.title}">!</span>` : '';
    const subtitle = n.type !== n.name ? n.type : (CATALOG[role+'s']?.find(c => c.type === n.type)?.desc || '');
    return `
  ...
  function pipelineYaml(model) {
    ...
    model.receivers.forEach(n => {
      ...
      Object.entries(n.cfg).forEach(([k,v]) => out.push(`    <span class="k">${k}:</span> <span class="s">${v}</span>`));
    });
    ...
    SIG_ORDER.forEach(sig => {
      const recv = new Set(), proc = [], exp = new Set();
      ...
      out.push(`      <span class="k">receivers:</span> <span class="s">[${[...recv].join(', ')}]</span>`);
      if (proc.length) out.push(`      <span class="k">processors:</span> <span class="s">[${proc.join(', ')}]</span>`);
      out.push(`      <span class="k">exporters:</span> <span class="s">[${[...exp].join(', ')}]</span>`);
    });
    return out.join('\n');
  }
  ...
  function renderPipeline(host, model, opts = {}) {
    ...
    host.querySelectorAll('.pipe-node').forEach(el => {
      el.addEventListener('click', (e) => {
        e.stopPropagation();
        host.querySelectorAll('.pipe-node[data-selected="1"]').forEach(n => n.removeAttribute('data-selected'));
    ...
    if (editable) {
      ...
      ['receivers','processors','exporters'].forEach(role => {
        ...
        col.addEventListener('dragover', e => { e.preventDefault(); col.style.background = 'color-mix(in oklab, var(--accent) 8%, transparent)'; });
        ...
        col.addEventListener('drop', e => {
          ...
          if (parsed.role + 's' !== role) {
            opts.onError && opts.onError(`${parsed.type} is a ${parsed.role}, not a ${role.slice(0,-1)}`);
            return;
          }
  ...
  function renderInspector(host, node, opts = {}) {
    ...
    const fieldHTML = fields.map(([k, v], i) => `
      <div class="field">
        <label>${k}</label>
        ${editable
    ...
    if (editable) {
      ...
      host.querySelectorAll('[data-cfg-key]').forEach(el => {
        el.addEventListener('input', () => opts.onCfgChange && opts.onCfgChange(el.getAttribute('data-cfg-key'), el.value));
      });
      host.querySelector('[data-act="delete"]')?.addEventListener('click', () => opts.onDelete && opts.onDelete());
      host.querySelector('[data-act="duplicate"]')?.addEventListener('click', () => opts.onDuplicate && opts.onDuplicate());
    }
  }
  ...
  window.OPipeline = {
    CATALOG, EXAMPLES, PIPELINE_INSIGHTS,
    renderPipeline, drawWires, attachResizeRedraw,
    renderInspector, renderPalette,
    pipelineYaml, findNode, newNode,
  };
})();
