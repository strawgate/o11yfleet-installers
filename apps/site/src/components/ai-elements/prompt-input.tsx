...
const convertBlobUrlToDataUrl = async (url: string): Promise<string | null> => {
  try {
    ...
    return new Promise((resolve) => {
      ...
      reader.onerror = () => {
        if (import.meta.env.DEV) {
          console.warn("Failed to read prompt attachment blob", { url, error: reader.error });
        }
        resolve(null);
      };
    ...
    if (import.meta.env.DEV) {
      console.warn("Failed to convert prompt attachment blob URL", { url, error });
    }
    return null;
  }
};
  ...
  const submitForm = useCallback(
    async (form: HTMLFormElement, event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        // Callers that perform async work should return their Promise so input clearing waits.
