...
Use `browser_snapshot` for accessible structure, screenshots for evidence, and `browser_run_code` to batch small known interactions. Do not retry the same failed action more than twice without changing your approach.
...
When signing in from a direct login URL, use `?api=http://localhost:8787` once, for example `/login?api=http://localhost:8787` and `/admin/login?api=http://localhost:8787`. After login, navigate normal app routes without repeating `?api=`. If the app loses its local API context after a normal in-app navigation, report that as a product or harness finding with evidence.
...
- Sign in through `/login?api=http://localhost:8787`
...
- Sign in through `/admin/login?api=http://localhost:8787`
