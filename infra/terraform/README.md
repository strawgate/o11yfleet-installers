...
| <a name="input_worker_crons"></a> [worker\_crons](#input\_worker\_crons) | Cron schedules for Terraform-managed Worker cron triggers. Must match `triggers.crons` in apps/worker/wrangler.jsonc — drift is checked by scripts/check-cron-drift.ts and the same script wired into CI's fast job. | `list(string)` | <pre>[<br/>  "0 0 * * *",<br/>  "17 3 * * *",<br/>  "12 6 * * *"<br/>]</pre> | no |
